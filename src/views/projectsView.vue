    ...
    <div w-full max-w-4xl p-4 md:p-10>
      <section>
        ...
        <div grid md:grid-cols-2 gap-4>
          <div v-for="(project) in projects" :key="project.id" class="bg-white-500/10 dark:bg-black-500/10 backdrop-blur-sm border border-black/10 dark:border-white/10 dark:border-black/10 p-6 rounded-3xl shadow-lg flex flex-col hover:border-white/20 justify-between">
            <div>
              <!-- Links -->
              <div flex items-center justify-between mb-2>
                ...
                <div :class="project.link && project.github || project.link && project.npm || project.npm && project.github 
                  ...
                  <a 
                    ...
                    :href="project.link" 
                    target="_blank" 
                    class="i-solar:eye-bold w-6 h-6 p-2 rounded-lg bg-black dark:bg-white hover:bg-black/50 dark:hover:bg-gray-300/50 backdrop-blur-sm transition"
                    title="Live Site"
                  />
                  <a 
                    v-if="project.github" 
                    :href="project.github" 
                    target="_blank" 
                    class="i-carbon:logo-github w-6 h-6 p-2 rounded-lg bg-black dark:bg-white hover:bg-black/50 dark:hover:bg-gray-300/50 backdrop-blur-sm transition"
                    title="GitHub Repo"
                  />
                  <a 
                    v-if="project.npm" 
                    :href="project.npm" 
                    target="_blank" 
                    class="i-carbon:logo-npm w-6 h-6 p-2 rounded-lg bg-black dark:bg-white hover:bg-black/50 dark:hover:bg-gray-300/50 backdrop-blur-sm transition"
                    title="Npm Package"
                  />
                </div>
              </div>
