...
import { getCaseStudyBySlug, type CaseBuiltItem, type CaseIncident } from '../utils/caseStudies'
import { createCaseStudySEO, notFoundSEO } from '../utils/tags'
...
useHead(computed(() => (cs.value ? createCaseStudySEO(cs.value) : notFoundSEO)))
...
<template>
  <FloorSection level="L-03" name="Project / Deep Dive" elevation="+0.00 m" :top-slab="false" eager>
    <div class="min-h-screen text-black dark:text-white">
        ...
        <header class="mt-12 md:mt-16">
          ...
          <dl class="case-rule mt-12 grid grid-cols-1 gap-8 pt-8 sm:grid-cols-3">
            <div>
              ...
              <dd class="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-300">{{ cs.role }}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd class="mt-3 flex flex-wrap gap-x-4 gap-y-2">
                ...
                >
                  <i
                    v-if="getTagIcon(tech)"
                    :class="getTagIcon(tech)!"
          ...
          <ul class="mt-8 space-y-6">
            <li v-for="post in relatedPosts" :key="post.slug">
              <router-link
                ...
                class="group block"
              >
                <span class="block text-lg font-semibold tracking-tight transition-opacity group-hover:opacity-70">
                  {{ post.title }}
                </span>
      ...
      <!-- Unknown slug -->
      <section v-else class="mx-auto w-full max-w-3xl px-4 py-20 text-center md:px-6">
        <h1 class="text-3xl font-semibold">Deep dive not found</h1>
        <p class="mt-3 text-gray-700 dark:text-gray-300">This project does not exist or was moved.</p>
        <router-link :to="'/projects'" class="bp-tab is-active mt-6 inline-flex items-center gap-2">
          <i class="i-carbon:arrow-left w-4 h-4" aria-hidden="true" />
          Back to projects
        </router-link>
      </section>
