...
import { notFoundSEO } from "~~/shared/utils/seo/shared";

// Nuxt renders this for any unmatched route / thrown createError. The static
// build prerenders it to 404.html, which Vercel/Nginx serve with a real 404.
...
const requestedPath = computed(() => {
  const candidate = props.error?.url || `${url.pathname}${url.search}` || sitePaths.home;
    ...
    >
      ...
      <div class="panel-enter relative z-10 space-y-6" style="animation-delay: 0.05s">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <button type="button" class="action-button primary-action" @click="handleBack">
            <i class="i-carbon:arrow-left text-lg"></i>
            Go Back
          </button>

          <NuxtLink :to="sitePaths.home" class="action-button ghost-action">
...
.not-found-shell::after {
  background-image: linear-gradient(var(--nf-grid) 1px, transparent 1px),
